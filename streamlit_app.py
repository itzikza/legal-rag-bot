...
import pypdf
import time  # נדרש לצורך המתנה בין בקשות

# --- LEXIS AI: PRO ARCHITECT EDITION ---
...
st.markdown("""
    ...
    .brand-title { font-size: 5.5rem; font-weight: 800; letter-spacing: -4px; background: linear-gradient(135deg, #ffffff 0%, #777777 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-align: center; margin-bottom: 0.5rem; line-height: 1; }
    .static-glass-card { background: rgba(255, 255, 255, 0.03); backdrop-filter: blur(20px); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 32px; padding: 45px; margin-bottom: 30px; text-align: left; }
    ...
    .feature-chip { border: 1px solid rgba(255, 255, 255, 0.4); padding: 12px 24px; border-radius: 100px; font-size: 0.9rem; font-weight: 600; color: #ffffff !important; background: rgba(255, 255, 255, 0.02); display: inline-block; margin-right: 15px; transition: all 0.3s ease; }
    .chat-header { font-size: 2.2rem; font-weight: 800; color: #ffffff !important; letter-spacing: -1.5px; margin-bottom: 20px; text-transform: uppercase; }
    div.stButton > button { background: rgba(255, 255, 255, 0.03); color: #ffffff !important; border: 1px solid rgba(255, 255, 255, 0.15); border-radius: 20px; padding: 2.5rem 1rem; font-weight: 800; font-size: 1.1rem; height: 110px; width: 100%; transition: all 0.4s ease; }
    div.stButton > button:hover { border-color: #ffffff !important; transform: translateY(-8px); box-shadow: 0 0 30px rgba(255, 255, 255, 0.3) !important; }
    .footer-white-btn { background: #ffffff !important; color: #000000 !important; padding: 18px 45px; border-radius: 100px; font-weight: 800; text-decoration: none; display: inline-block; }
    .search-container { margin-bottom: 4rem; }
...
def embed_text(text):
    """פונקציה ליצירת וקטורים עם טיפול בשגיאות מכסה (Rate Limit)"""
    try:
        ...
        if "429" in str(e):
            st.warning("Quota reached. Waiting 10 seconds to resume...")
            time.sleep(10)  # המתנה של 10 שניות במקרה של חסימה
            return embed_text(text)  # ניסיון חוזר
        raise e
...
for msg in reversed(st.session_state.messages):
    ...
    st.markdown(f"<div class='message-glass-card' style='border-color: rgba(255,255,255,0.3);'><div class='chat-header'>{header}</div><div style='font-size: 1.4rem;'>{msg['content']}</div></div>", unsafe_allow_html=True)

# --- 4. NEURAL VAULT (With Intelligent Sleep) ---
st.markdown("---")
                ...
                for f in uploaded_files:
                    with st.spinner(f"Indexing {f.name}..."):
                        ...
                        for i, chunk in enumerate(chunks):
                            ...
                            # הוספת המתנה קצרה כדי לא לחרוג מהמכסה (3 בקשות בשנייה)
...
st.markdown("<div style='text-align: center; padding: 120px 0; border-top: 1px solid rgba(255, 255, 255, 0.05);'><div style='font-size: 4rem; font-weight: 800; margin-bottom: 25px;'>Let's redefine the law.</div><div style='display: flex; justify-content: center; gap: 30px;'><a href='#' class='footer-white-btn'>Connect on LinkedIn</a></div></div>", unsafe_allow_html=True)
